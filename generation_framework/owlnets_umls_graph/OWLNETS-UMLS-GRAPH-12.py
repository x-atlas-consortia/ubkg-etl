...
if 'firstname' not in node_metadata.columns:
    node_metadata['firstname'] = np.nan
    node_metadata['lastname'] = np.nan
    node_metadata['email'] = np.nan
...
#    b. the existing UMLS CUI of the node
#    c. the non-UMLS CUI of a cross-reference (previously ingested in the ontology CSVs from a non-UMLS ontology)
...
# These will be written to a calculated field named "nodeCUIs".
...
# Note that this will be all cross-references, not just UMLS CUIs.
...
# Identify cross-references that are UMLS CUIs. Write to field named nodeCUIs.
...
# This contains the assignments from CUI to code that existed prior to the current ingestion.
# There can be multiple assignment rows for each CUI.
...
# Use groupby to convert the DataFrame from CUI_CODES (assignments from concept to code)
# to a DataFrame of assignments from code to concept.
...
ulog.print_and_logger_info('-- Converting CUI-CODE assignments to CODE-CUI assignments...')
...
ulog.print_and_logger_info('-- Merging CODE-CUIs with node metadata...')
...
# Identify CUIs for non-UMLS nodes that were previously ingested.
# original comment:
...
# AUG 2025 Add submitter columns.
...
def getnewsuisfortermtype(termtype: str, owlsab: str, dfnode: pd.DataFrame, dfsuis: pd.DataFrame, dfcodesuis: pd.DataFrame) -> pd.DataFrame:
    ...
    #
    # This function distinguishes between specified terms from an "owning ontology" from
    ...
    # ingesting ontology. To eliminate unnecessary duplication, the script only adds a "_SAB" term
